-- FUNCTION: public.click_guardar_pedido_prime(integer, character, character, integer, character, character, character, integer, integer, character varying, character varying, character, integer, boolean)

-- DROP FUNCTION IF EXISTS public.click_guardar_pedido_prime(integer, character, character, integer, character, character, character, integer, integer, character varying, character varying, character, integer, boolean);
...
AS $BODY$
	...
	BEGIN
	  ...
	  LOOP 
	      ...
	      LOOP	     
	         ...
	         INSERT INTO sale_order(id, name, state, date_order, create_date, partner_id, partner_invoice_id, partner_shipping_id, pricelist_id, invoice_status, note, amount_untaxed, amount_tax, amount_total, company_id, team_id,
	                                ...
	                                national_sale_foil,origin_warehouse_id,parcel_route_id,national_sale_bool,payment_term_month_id,is_prime)
	         values (ifolio, iname, 'draft', now(), now(), icliente, icliente, ipartner_shopping_id, 1,'no', icomentarios, 0, 0,0,1,id_crm_team,
	                 ...
	                 ifolio_nac,iWarehouse_id,row_sucursal.parcel_route_id,sale_bool,ipayment_term_month_id,bpedido_prime);  
		       ...
		       FOR linea IN SELECT ifolio as id,cc.codigo_art,cc.cantidad,cc.surtido,CASE WHEN cc.precio_oferta > 0 THEN cc.precio_oferta ELSE cc.precio END AS precio,cc.cliente::char AS cliente,cc.sucursal_id as sucursal,cc.flag_urgente,
	                      pro.uom_id,pp.id as product_id,pro.supplier_id,pro.line_id,pro.currency_two_id,tax.amount::numeric as iva,tax.id as tax_id FROM click_carrito cc
	    	   ...
	    	   LOOP
	            ...
	            INSERT INTO sale_order_line(id,order_id, name,invoice_status,price_unit,price_subtotal,price_tax,price_total,price_reduce,price_reduce_taxinc,price_reduce_taxexcl,discount,product_id,product_uom_qty,product_uom,qty_delivered,
	                        qty_to_invoice,qty_invoiced,currency_id,company_id, order_partner_id,is_downpayment,state,customer_lead,amt_to_invoice,amt_invoiced,create_date,write_date,is_delivery,supplier_id,line_id)
	            ...
	            desc_pronto,linea.product_id,linea.cantidad,linea.uom_id,0,0,0,linea.currency_two_id,1,icliente,false,'draft',0.0,0.00,0.00,now(),now(),false,linea.supplier_id,linea.line_id);
	            ...
	            IF binsertar_prime THEN
	               ...
	               LOOP
	                 ...
	                 VALUES(iorden,ifolio,row_prime.codigo_art,'no',row_prime.precio,/*price_unit*/row_prime.precio, /*price_subtotal*/row_prime.precio * (row_prime.iva / 100), /*price_tax*/
	                       ...
	                       desc_pronto,row_prime.product_id,1,row_prime.uom_id,0,0,0,row_prime.currency_two_id,1,icliente,false,'draft',0.0,0.00,0.00,now(),now(),false,row_prime.supplier_id,row_prime.line_id);
	  ...
	  --Respaldar carrito 
	  tparams:='';
      tparams:='cliente:'||icliente||' clave:'||iclave||' tipo_venta:'||itipo_venta||' flete:'||coalesce(iflete,0)||' comentarios:'||icomentarios||' tipo_usuario:'||itipo_usuario||' clave_cte:'||iclave_cte||' socio:'||isocio||' partner_shipping_id:'||ipartner_shopping_id||' tipo_envio:'||p_tipo_envio||' p_tipe_dist_morsa:'||p_tipe_dist_morsa||' metodo_pago:'||c_metodo_pago||' meses:'||coalesce(imeses,0)||' prime:'||bprime;
      insert into click_carrito_bak (cliente,product_id,clave_cte,codigo_art,cantidad,surtido,precio,precio_original,precio_oferta,oferta,flag_urgente,flag_actvo,tipo_usuario,registro,partner_shipping_id,sucursal_id,id_pedido,parametros)
      SELECT cliente,product_id,clave_cte,codigo_art,cantidad,surtido,precio,precio_original,precio_oferta,oferta,flag_urgente,flag_actvo,tipo_usuario,registro,partner_shipping_id,sucursal_id,ifolio,tparams 
...
ALTER FUNCTION public.click_guardar_pedido_prime(integer, character, character, integer, character, character, character, integer, integer, character varying, character varying, character, integer, boolean)
